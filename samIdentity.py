...
parser = argparse.ArgumentParser(
    description="Helper scrip to find alignmnet stats from a sam/bam/cram."
)
...
parser.add_argument(
    "--qbed",
    help="bed with query coordinates leading",
    action="store_true",
    default=False,
)
parser.add_argument(
    "--split",
    help="split chr:start-end into chr and then add start to other previous number",
    action="store_true",
    default=False,
)
parser.add_argument("--tag", help="ID tag to add to each row", default=None)
parser.add_argument(
    "--mask", help="a list of query names to add a mask flag == True for", default=None
)
...
if args.qbed:
    ...
    if args.split:
        ...
        # df=separate(df,query_name, "-|:", into=c("query_name","q_add","q_trash"))
        # df$q_add = as.numeric(df$q_add) -1
        # df$query_start = df$q_add + df$query_start
        # df$query_end = df$q_add + df$query_end
        tmp = df.query_name.str.split("-|:", expand=True)
        start = tmp[1].astype(int)
        df.query_name = tmp[0]
        df.query_start = df.query_start + start
        df.query_end = df.query_end + start
