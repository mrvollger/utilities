...
parser.add_argument(
    "inputFile",
    nargs="?",
    help="autodetects plus does regex, but the regex requires quotes around it",
    default=None,
)
...
parser.add_argument(
    "--printn", help="print the lenght of the first n reads", default="0"
)
parser.add_argument(
    "--hist", help="print a histogram of the lengths", action="store_true"
)
...
def runPls2Fasta(reads, outreads):
    ...
    proc = subprocess.Popen(
        cmd + cmd_options, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
