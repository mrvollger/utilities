...
parser.add_argument(
    "png", help="output png, the name must have the .png ext", default=None
)
parser.add_argument(
    "--m5",
    help="an m5 output from blasr that indicates a mapping of some sequences onto the depth profile",
    default=None,
)
parser.add_argument(
    "--sam",
    help="a sam or bam file of sequences to plot against the depth profile(contigs against a depth profile)",
    default=None,
)
...
def addPatchToAx(ax, x, y, width, height, operation):
    ...
    ax.add_patch(
        patches.Rectangle(
            (x, y), width, height, alpha=0.75, linewidth=0, color=col[operation]
        )
    )
